...
\end{equation}
Mit dem Wissen, dass der perfekte Wert $\frac{\chi^2}{doF}=1$ ist und der Annahme, dass die Fehler für alle Werte gleich groß sind ($\sigma_i=\sigma$), lassen sich die Fehler wie folgt berechnen
\begin{equation}
...
COV(X,X)=\sigma_X^2.
\end{equation}
Interessant ist auch sich die Korrelation der Parameter untereinander anzusehen. Dazu wird der Korrelationskoeffizient 
\begin{equation}
...
\label{img:D2C}
\end{figure}
so sieht man, dass für die beiden oberen Graphen die Effekte ähnlich zu denen in Abbildung \ref{img:C2D} sind. Dass die Graphen der Azimutabhängigkeit verzerrt aussehen, lässt sich durch den Wechsel der Koordinaten erklären. Vergleicht man wieder die Positionen der gemessenen Werte mit den Modellwerten (Abbildung \ref{img:D2Ccomp2}), so stellt man fest, das wie in Abbildung \ref{img:C2Dcomp} die Wellenbewegungen unabhängig von den einzelnen Werten der Elevation unabhängig sind.
\begin{figure}[htbp]
\centering
...
\subsection{Abhängigkeit der Drivekoordinaten in Abhängigkeit der CCD-Koordinaten}
Hier wird wie oben mit der Vorhersage der Drive-Koordinaten in Abhängigkeit der CCD-Koordinaten begonnen. Dazu verwendet man die Formeln \ref{eq:elC2D4} und \ref{eq:azC2D4}, für die sich mithilfe des Fit-Programms die Parameter aus Tabelle \ref{tab:C2D4} berechnen lassen.
\begin{table}[htbp]
\centering
\begin{tabular}{rcl}
\toprule
...
\end{equation}
ermittelt. Das Gleiche gilt für die Korrelation zwischen $az_0$ und $el_0$. Hier ist der Korrelationskoeffizient
\begin{equation}
\rho_{az_0,az_1}= -0,9526
\end{equation}
ähnlich groß. Da sich gerade in diesem Modell die Parameter kaum von denen des Zwei-Parameter-Modells unterscheiden (dort sind die zusätzlichen Paramter Null), erwartet man keine großen Unterschiede zwischen diesen Modellen. Diese These wird durch Abbildung \ref{img:D2C4comp} bestätigt.
\begin{figure}[htbp]
\centering
