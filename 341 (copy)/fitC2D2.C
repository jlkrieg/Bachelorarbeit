...
Double_t funcEl(Double_t el,Double_t az,Double_t az0,Double_t el0){
  ...
  az0 = az0*f;
  el0 = el0*f;
  double temp1 = cos(az0)-sqrt(pow(cos(az0),2)+pow(sin(el0),2)-pow(sin(el),2));
...
Double_t funcAz(Double_t el,Double_t az,Double_t az0,Double_t el0){
  ...
  double X = cos(az)*(cos(el)*cos(az0)-sin(el)*sin(el0))+sin(az)*sin(az0)*cos(el0);
  double Y = -sin(az)*(cos(el)*cos(az0)-sin(el)*sin(el0))+cos(az)*sin(az0)*cos(el0);
  return atan2(-Y,X)/f;
}
