...
//class pointing{
//pointing(Double_t el,Double_t az,Double_t az0,Double_t el0);
//az = az*f;
//el = el*f;
//az0 = az0*f;
//el0 = el0*f;
...
Double_t chi(Double_t az, Double_t el,Double_t azd, Double_t eld,Double_t az0, Double_t el0){
  Double_t result=0;
  ...
  Double_t N=sizeof(az)/sizeof(az)
  for(int i=0;i<N;i++){
      Double_t del = funcEl(el[i],az[i],az0,el0)-eld[i];
      if (del < -180){
	del+=360;
      }
      if (del > 180){
	del-=360;
      ...
      Double_t daz = funcAz(el[i],az[i],az0,el0)-azd[i];
      if (daz < -180){
	daz+=360;
      }
      if (daz > 180){
...
void fit(){
  ...
  minimizer->SetFCN(minuitFunction(az_vec,el_vec,azd_vec,eld_vec));
