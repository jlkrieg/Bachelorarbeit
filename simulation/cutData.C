...
void cutData(){
  ...
  Double_t u[N],azd[N],eld[N],az[N],el[N],flag[N],ps[N],x1[N],x2[N],x3[N];
  Int_t N_c=1;
  //TNtuple *ntuple = new TNtuple("nt", "nt","u:azd:eld:az:el:flag:ps:x1:x2:x3",32000);
  TNtuple *ntuple = new TNtuple("nt", "nt","azd:eld:azc:elc:flag:ps:x1:x2:x3",32000);
  ...
  for (int i=0; i<N; i++){
    ...
    x3[i]  = a[9];
    if (fabs(azd[i])<5){
      std::cout<<"azd"<<std::cout;
      if(fabs(eld[i])>35){
	if(fabs(ps[i]-11.03)<0.05){
	  N_c++;
  ...
  for (int i=0; i<N; i++){
    if (fabs(azd[i])<5 && fabs(eld[i])>35 && fabs(ps[i]-11.03)<0.05){
      ...
      ntuple->TNtuple::Fill(azd_c[i_c],eld_c[i_c],az_c[i_c],el_c[i_c],flag_c[i_c],ps_c[i_c],x1_c[i_c],x2_c[i_c],x3_c[i_c]);
