...
void fit281(){
  ...
  TGraph2D* g1fit=new TGraph2D(kk,el_vec,az_vec,eld_vec);
  //g1fit->Sort();
  //can->cd(1);
  TF2*f1 = new TF2("fel",fitEl,0,90,2);
  g1fit->Fit("fel","","",20,90);
  ...
  //can->cd(2);
  TGraph2D* g2fit=new TGraph2D(kk,el_vec,az_vec,azd_vec);
  TF2*f2 = new TF2("faz",fitAz,0,90,2);
  g2fit->Fit("faz","","",20,90);
